...
pub mod pallet {
	...
	pub type NextKttyId<T> = StorageValue<_, KittyId, ValueQuery>;
	...
	pub enum Event<T: Config> {
		KittyCreaded { who: T::AccountId, kitty_id: KittyId, kitty:Kitty },
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create(origin: OriginFor<T>) -> DispatchResult {
			...
			Self::deposit_event(Event::KittyCreaded { who, kitty_id, kitty });
	...
	impl<T: Config> Pallet<T> {
		fn get_next_id() -> Result<KittyId, DispatchError> {
			NextKttyId::<T>::try_mutate(|next_id| -> Result<KittyId, DispatchError> {
